...
```bash
DRIFT_RENDEZVOUS_URL=http://127.0.0.1:8787 cargo run --bin drift -- send sample.txt photos/
```

3. Accept the offer on the receiver:

```bash
DRIFT_RENDEZVOUS_URL=http://127.0.0.1:8787 cargo run --bin drift -- receive AB2CD3 --out downloads
```
...
```bash
cargo run --bin drift -- receive-ticket --out downloads
cargo run --bin drift -- send-ticket <ticket> sample.txt photos/
```
