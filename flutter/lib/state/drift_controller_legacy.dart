...
class DriftController extends ChangeNotifier {
  DriftController({
    ...
    if (_receiverServiceController != null) {
      _receiverServiceController.addListener(_syncFromReceiverService);
      _syncFromReceiverService();
    } else if (enableIdleReceiverRegistrationBootstrap) {
      unawaited(_ensureIdleReceiver());
    }
    if (_receiverServiceController == null &&
        enableIdleReceiverRegistrationBootstrap &&
        enableIdleReceiverRefresh) {
  ...
  final ReceiverServiceController? _receiverServiceController;
  ...
  TransferStage get receiveStage =>
      _serviceIncomingSession?.stage ?? _receiveStage;
  ...
  String? get receiveErrorText =>
      _serviceIncomingSession?.errorText ?? _receiveErrorText;
  List<TransferItemViewData> get sendItems => _sendItems;
  bool get isInspectingSendItems => _isInspectingSendItems;
  List<TransferItemViewData> get receiveItems =>
      _serviceIncomingSession?.items ?? _receiveItems;
  ...
  TransferSummaryViewData? get receiveSummary =>
      _serviceIncomingSession?.summary ?? _receiveSummary;
  ...
  int? get receivePayloadBytesReceived =>
      _serviceIncomingSession?.payloadBytesReceived ??
      _receivePayloadBytesReceived;
  int? get receivePayloadTotalBytes =>
      _serviceIncomingSession?.payloadTotalBytes ?? _receivePayloadTotalBytes;
  ...
  List<TransferMetricRow>? get receiveCompletionMetrics {
    if (receiveStage != TransferStage.completed || receiveSummary == null) {
      return null;
    }
    final s = receiveSummary!;
    final startedAt =
        _serviceIncomingSession?.payloadStartedAt ?? _receivePayloadStartedAt;
    final now = DateTime.now();
    final bytesReceived = receivePayloadBytesReceived ?? 0;
  ...
  List<TransferItemViewData> get activeItems =>
      _mode == TransferDirection.receive ? receiveItems : _sendItems;
  ...
  bool get hasActiveTransferCard =>
      _sendStage != TransferStage.idle || receiveStage != TransferStage.idle;
  bool get hasSendFlow => _sendStage != TransferStage.idle;
  bool get hasReceiveFlow => receiveStage != TransferStage.idle;
  ...
  bool get showShellBackButton =>
      ...
      !(_mode == TransferDirection.receive &&
          receiveStage == TransferStage.waiting);

  ReceiverIncomingSessionState? get _serviceIncomingSession =>
      _receiverServiceController?.incomingSession;
  ...
  void _syncFromReceiverService() {
    final service = _receiverServiceController;
    if (service == null) {
      return;
    }
    syncReceiverBadge(
      code: service.badgeState.code,
      status: service.badgeState.status,
    );
    if (service.incomingSession != null) {
      _mode = TransferDirection.receive;
  ...
  void acceptReceiveOffer() {
    final service = _receiverServiceController;
    if (service?.incomingSession?.isDecisionPending ?? false) {
      _mode = TransferDirection.receive;
      _receiveEntryExpanded = true;
      notifyListeners();
      unawaited(service!.acceptIncomingOffer());
      return;
    }
  ...
  void declineReceiveOffer() {
    ...
    if (service?.incomingSession?.isDecisionPending ?? false) {
      _receiveEntryExpanded = false;
      notifyListeners();
      unawaited(service!.declineIncomingOffer());
      return;
    }
    ...
    if (_mode == TransferDirection.receive) {
      switch (receiveStage) {
        case TransferStage.review:
          if (_receiverServiceController?.incomingSession != null) {
            _receiveEntryExpanded = false;
            notifyListeners();
            unawaited(_receiverServiceController!.declineIncomingOffer());
            return;
          }
          if (_idleIncomingDecisionPending) {
            unawaited(_respondIdleIncoming(accept: false));
        ...
        case TransferStage.completed:
          _receiverServiceController?.clearIncomingSession();
          _receiveStage = TransferStage.idle;
          _receiveEntryExpanded = false;
          _receiveErrorText = null;
          _receiveItems = const [];
  ...
  @override
  void dispose() {
    _receiverServiceController?.removeListener(_syncFromReceiverService);
    _cancelNearbyScanTimer();
    _cancelActiveSendTransfer();
  ...
  Future<void> _respondIdleIncoming({required bool accept}) async {
    try {
      if (_receiverServiceController != null) {
        await _receiverServiceController.respondToOffer(accept: accept);
      } else {
  ...
  Future<void> _pauseIdleLanAdvertisementForActiveSend() async {
    try {
      if (_receiverServiceController != null) {
        await _receiverServiceController.setDiscoverable(enabled: false);
      } else {
  ...
  Future<void> _resumeIdleLanAdvertisementAfterSend() async {
    try {
      if (_receiverServiceController != null) {
        await _receiverServiceController.setDiscoverable(enabled: true);
      } else {
