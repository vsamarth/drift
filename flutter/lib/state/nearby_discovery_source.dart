...
class LocalNearbyDiscoverySource implements NearbyDiscoverySource {
  ...
  Future<List<SendDestinationViewData>> scan({
    ...
    debugPrint(
      '[NearbyDiscoverySource] starting scan (timeout: ${timeout.inSeconds}s)',
    );
    try {
      ...
      for (final receiver in raw) {
        debugPrint(
          '[NearbyDiscoverySource]   - raw receiver: ${receiver.fullname} (label: ${receiver.label})',
        );
      ...
      debugPrint(
        '[NearbyDiscoverySource] returning ${items.length} mapped destinations',
      );
